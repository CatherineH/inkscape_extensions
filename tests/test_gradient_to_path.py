...
class TestGradientToPath(TestCase):
    ...
    def test_basic(self):
        ...
        assert effect.gradient.stops[2].style == inkex.Style(
            [("stop-color", "blue"), ("stop-opacity", "1.0")]
        )
        assert effect.gradient.stops[0].style == inkex.Style(
            [("stop-color", "red"), ("stop-opacity", "1.0")]
        )
        print(effect.sample_color(inkex.transforms.Vector2d(10, 120)))
        print(effect.sample_color(inkex.transforms.Vector2d(10, 220)))
        assert effect.sample_color(
            inkex.transforms.Vector2d(10, 120), debug=True
        ) == inkex.Style([("stop-color", "red"), ("stop-opacity", "1.0")])
        assert effect.sample_color(
            inkex.transforms.Vector2d(10, 220), debug=True
        ) == inkex.Style([("stop-color", "blue"), ("stop-opacity", "1.0")])
    ...
    def test_css_trct(self):
        ...
        assert effect.sample_color(inkex.transforms.Vector2d(10, 10)) == inkex.Style(
            [("stop-color", "red"), ("stop-opacity", "1.0")]
        )
        assert effect.sample_color(inkex.transforms.Vector2d(1100, 10)) == inkex.Style(
            [("stop-color", "blue"), ("stop-opacity", "1.0")]
        )
